...
--Testcase 429:
EXPLAIN (verbose, costs off)
UPDATE ft2 SET c3 = 'bar' WHERE mysql_fdw_abs(c1) > 2000;            -- can't be pushed down
--Testcase 430:
SELECT * FROM ft2 WHERE mysql_fdw_abs(c1) > 2000;
--Testcase 431:
UPDATE ft2 SET c3 = 'bar' WHERE mysql_fdw_abs(c1) > 2000;
--Testcase 432:
SELECT * FROM ft2 WHERE mysql_fdw_abs(c1) > 2000;
--Testcase 433:
EXPLAIN (verbose, costs off)
...
--Testcase 481:
CREATE VIEW rw_view AS SELECT a, b FROM foreign_tbl
...
-- ===================================================================
--Testcase 515:
create foreign table loc1 (id int, f1 serial, f2 text)
  server mysql_svr options(dbname 'mysql_fdw_post', table_name 'loc1');

--Testcase 516:
create foreign table rem1 (id int, f1 serial, f2 text)
  server mysql_svr options(dbname 'mysql_fdw_post', table_name 'loc1');
--Testcase 517:
...
-- ===================================================================
--Testcase 684:
create foreign table rem2 (id int, f1 int, f2 text) server mysql_svr options(dbname 'mysql_fdw_post', table_name 'loc2');
...
-- Test check constraints
--Testcase 687:
create foreign table rem4 (id int, f1 int, f2 text) server mysql_svr options(dbname 'mysql_fdw_post', table_name 'loc4');
--Testcase 688:
alter foreign table rem4 add constraint rem4_f1positive check (f1 >= 0);

-- check constraint is enforced on the remote side, not locally
copy rem4 from stdin;
1	1	foo
2	2	bar
\.
copy rem4 from stdin; -- ERROR
3	-1	xyzzy
\.
--Testcase 689:
select * from rem4;
...
--Testcase 690:
alter foreign table rem4 drop constraint rem4_f1positive;
--Testcase 691:
drop foreign table rem4;
...
begin;
--Testcase 710:
create foreign table rem3 (id int, f1 int, f2 text)
...
--Testcase 844:
CREATE FOREIGN TABLE ftable ( x int ) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_post', table_name 'batch_table', batch_size '10' );
--Testcase 845:
...
--Testcase 852:
CREATE FOREIGN TABLE ftable ( x int ) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_post', table_name 'batch_table', batch_size '100000' );
--Testcase 853:
...
--Testcase 856:
CREATE FOREIGN TABLE ftable ( x int ) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_post', table_name 'batch_table', batch_size '1' );
--Testcase 857:
